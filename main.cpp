...
SC_MODULE(can_node)
{
	...
	void transmit(void)
	{
		while(true)
		{
			...
			std::cout << "["<<name()<<"]" << "@ " << sc_time_stamp() << "  TX(success/total) " << success_tx <<  "/" << total_tx << std::endl;
			...
			if( (((time > 0) && (time%20000)==0) && ( strcmp(name(),"node3")==0) && (bus_tx.read() == 0)) || (failed_tx && ( strcmp(name(),"node3")==0))) 
			{
	...
	void entry_func(void)
	{
		...
		while(true) {
			...
			if(ongoing_reception) {	
				...
				cout << "the winner this round is : index: " << winner <<endl;
